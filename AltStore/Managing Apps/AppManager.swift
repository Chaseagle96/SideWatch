        ...
        DispatchQueue.main.async {
            ...
            if UserDefaults.standard.activeAppLimitIncludesExtensions
            {
                ...
                else
                {
                    ...
                    let appExtensionCount = appBundle.appIDCount - 1
                    let appExtensionText = appExtensionCount == 1 ? NSLocalizedString("embedded app", comment: "") : NSLocalizedString("embedded apps", comment: "")
                    message = String(format: NSLocalizedString("Non-developer Apple IDs are limited to 3 active apps and embedded apps, and \"%@\" contains %@ %@. Please choose an app to deactivate.", comment: ""), appBundle.name, NSNumber(value: appExtensionCount), appExtensionText)
                }
            }
            else
            {
            ...
            let requiredActiveSlots = UserDefaults.standard.activeAppLimitIncludesExtensions ? appBundle.appIDCount : 1
